    ...
    <>
      ...
      {isOpen && (
        <div className="lg:hidden fixed inset-0 z-50 bg-black/95 flex flex-col">
          <div className="sticky top-0 z-10 flex justify-end p-4 shrink-0 bg-black/95 border-b border-slate-800">
            <button
              onClick={() => setIsOpen(false)}
              aria-label="Close demo and return to Case Studies"
              className="flex items-center gap-2 text-white bg-slate-800 rounded-full pl-4 pr-3 py-2"
            >
              <span className="text-xs font-black uppercase tracking-widest">Back to Case Studies</span>
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          </div>
          <div className="flex-1 overflow-y-auto px-4 pt-4 pb-16 flex items-start justify-center">
            {children}
          </div>
        </div>
      )}
