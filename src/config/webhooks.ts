// Central place for every Make.com webhook URL used on this site.
// Update the value here once and every form that imports it picks up the change —
// no more hunting through components for 'YOUR_MAKE_WEBHOOK_URL_HERE'.

export const CONTACT_WEBHOOK_URL = 'https://hook.us2.make.com/8rfha8cgdlsmtnrcntl0yipapnykx9je';
export const SITE_PROGRESS_WEBHOOK_URL = 'https://hook.us2.make.com/kkth7gf6ldafbvwo2y4squd3chvxnj6w';
export const MAINTENANCE_WEBHOOK_URL = 'https://hook.us2.make.com/c1afkyv1s24u1zys40nv8zu6yzqbbfqd';
export const SHOWING_WEBHOOK_URL = 'https://hook.us2.make.com/dyk9yh1vnzho0icm9djhdxvx5qyyt6lk';
export const ASSET_DROP_OFF_WEBHOOK_URL = 'https://hook.us2.make.com/uy2blad99b3bqxnxrt1k6mq3qysjvkis';
export const ASSET_RETURN_WEBHOOK_URL = 'https://hook.us2.make.com/87znwz8ftrqpsmfwc49bf533g2kufs0l';
export const ASSET_GET_LOCATIONS_WEBHOOK_URL = 'https://hook.us2.make.com/ohd5tlttrqstc7y4cg9377fwkncy5scv';
